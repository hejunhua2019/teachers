...
<html>
	<head>
		...
		<script type="text/javascript">
			...
			var deleteUrl="user/deleteUpdateAll";
			var addUrl="user/add";
			var url;
				...
				*/
				...
				$(datagridId).datagrid({  
					...
					columns:[[
						...
						{field:'account',title:'账号',width:60},
						{field:'password',title:'密码',width:60},
						{field:'token',title:'token',width:60},
			...
			function edit(){
				...
				if (rows.length > 1) {
					$.messager.alert('提示', "只能选择一个用户进行更新", 'info');
					return;
				}
								
				var row = $(datagridId).datagrid('getSelected');
				if (row){
					$.get("user/edit",{id:row.id},function(result){
						if (result["success"]==true){							
		...
		<div  class="easyui-panel"  title="用户管理" >			
			...
			<div id="toolbar_listTable" style="padding:5px;height:auto">
				...
				<form action="" id="searchForm">
					<div>
						账号: <input name="account" style="width:60px">								
						<a class="easyui-linkbutton" iconCls="icon-search" onclick="javascript:doSearch()">查询</a>
						<c:if test="${true || wface:contains(upList,'46') }">
							<a class="easyui-linkbutton" iconCls="icon-add" onclick="javascript:add()">新增用户</a>
						</c:if>
						<c:if test="${true || wface:contains(upList,'47') }">
							<a class="easyui-linkbutton" iconCls="icon-edit" onclick="javascript:edit()">修改用户</a>
						</c:if>
						<c:if test="${true || wface:contains(upList,'48') }">
							<a class="easyui-linkbutton" iconCls="icon-remove" onclick="javascript:removeAll()">删除用户</a>
						</c:if>
					</div>
				</form>
			</div>
			...
			<div id="addoreditdlg" title="  " class="easyui-dialog" data-options="draggable:true,resizable:true" style="width:500px;height:430px;padding:10px 20px" closed="true" buttons="#addoreditdlg-buttons">
				...
				<form id="addoreditfm" method="post" novalidate>
					<div class="fitem">
						<label>账号:</label>
						<input id="account" name="account" class="easyui-validatebox" required="true"  ><br/>
						<label>密码:</label>
						<input id="password" name="password" class="easyui-validatebox" required="true"  ><br/>
						<label>token:</label>
						<input id="token" name="token" class="easyui-validatebox" required="true"  ><br/>
						<label>类型:</label>
