...
<html>
	<head>
		...
		<script type="text/javascript">
			...
			var pageSize=20;
			var listUrl="trafficPolice/listpage";
			var updateUrl="trafficPolice/update";
			var deleteUrl="trafficPolice/deleteUpdateAll";
			var addUrl="trafficPolice/add";
			var url;
			...
			$(function() {
				...
				$(datagridId).datagrid({  
					...
					nowrap:true,
					url:"trafficPolice/listpage",
					columns:[[
						{field:'ck',title:'',checkbox:true,width:2},
						{field:'name',title:'姓名',width:60},
						{field:'policeID',title:'警号',width:60},
						{field:'userID',title:'用户ID',width:60},
						{field:'departmentID',title:'部门ID',width:60},
					]],
						...
						if (r){
							$.get(deleteUrl,{ids:ids},function(result){
								if (result["success"]==true){
									$(datagridId).datagrid('reload'); // reload the trafficPolice data
								} else {
			...
			function edit(){
				function switch2edit(){
					url=updateUrl;
					$(addOrEditFormId).form("reset");//重置表单
					$("#addoredittitle").html('编辑交警');
				}
				switch2edit();				
				
				var rows = $(datagridId).datagrid('getSelections');
				if (rows.length == 0) {
					$.messager.alert('提示', "请选择你要更新的交警", 'info');
					return;
				}
				if (rows.length > 1) {
					$.messager.alert('提示', "只能选择一个交警进行更新", 'info');
					return;
				}
								
				var row = $(datagridId).datagrid('getSelected');
				if (row){
					$.get("trafficPolice/edit",{id:row.id},function(result){
						if (result["success"]==true){							
	...
	<body>
		...
		<div  class="easyui-panel"  title="交警管理" >			
			...
			<div id="toolbar_listTable" style="padding:5px;height:auto">
				...
				<form action="" id="searchForm">
					<div>
						姓名: <input name="name" style="width:60px">
						警号: <input name="policeID" style="width:60px">
						用户ID: <input name="userID" style="width:60px">
						部门ID: <input name="departmentID" style="width:60px">
						<a class="easyui-linkbutton" iconCls="icon-search" onclick="javascript:doSearch()">查询</a>
						<c:if test="${true || wface:contains(upList,'46') }">
							<a class="easyui-linkbutton" iconCls="icon-add" onclick="javascript:add()">新增交警</a>
						</c:if>
						<c:if test="${true || wface:contains(upList,'47') }">
							<a class="easyui-linkbutton" iconCls="icon-edit" onclick="javascript:edit()">修改交警</a>
						</c:if>
						<c:if test="${true || wface:contains(upList,'48') }">
							<a class="easyui-linkbutton" iconCls="icon-remove" onclick="javascript:removeAll()">删除交警</a>
						</c:if>
					</div>
				</form>
			</div>
			...
			<div id="addoreditdlg" title="  " class="easyui-dialog" data-options="draggable:true,resizable:true" style="width:500px;height:430px;padding:10px 20px" closed="true" buttons="#addoreditdlg-buttons">
				...
				<form id="addoreditfm" method="post" novalidate>
					<div class="fitem">
						...
						<label>警号:</label>
						<input id="policeID" name="policeID" class="easyui-validatebox" required="true"  ><br/>
						<label>用户ID:</label>
						<input id="userID" name="userID" class="easyui-validatebox" required="true"  ><br/>
						<label>部门ID:</label>
						<input id="departmentID" name="departmentID" class="easyui-validatebox" required="true"  ><br/>
					</div>					
