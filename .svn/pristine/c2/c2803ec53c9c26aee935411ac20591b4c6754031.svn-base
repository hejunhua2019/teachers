...
<html>
	<head>
		...
		<script type="text/javascript">
			...
			var pageSize=20;
			var listUrl="department/listpage";
			var updateUrl="department/update";
			var deleteUrl="department/deleteUpdateAll";
						...
						if (r){
							$.get(deleteUrl,{ids:ids},function(result){
								if (result["success"]==true){
									$(datagridId).datagrid('reload'); // reload the department data
								} else {
			...
			function edit(){
				function switch2edit(){
					url=updateUrl;
					$(addOrEditFormId).form("reset");//重置表单
					$("#addoredittitle").html('编辑部门');
				}
				switch2edit();				
				
				var rows = $(datagridId).datagrid('getSelections');
				if (rows.length == 0) {
					$.messager.alert('提示', "请选择你要更新的部门", 'info');
					return;
				}
				if (rows.length > 1) {
					$.messager.alert('提示', "只能选择一个部门进行更新", 'info');
					return;
				}
								
				var row = $(datagridId).datagrid('getSelected');
				if (row){
					$.get("department/edit",{id:row.id},function(result){
						if (result["success"]==true){							
		...
		<div  class="easyui-panel"  title="部门管理" >			
			...
			<div id="toolbar_listTable" style="padding:5px;height:auto">
				...
				<form action="" id="searchForm">
					<div>
						地址: <input name="addr" style="width:60px">
						...
						<c:if test="${true || wface:contains(upList,'46') }">
							<a class="easyui-linkbutton" iconCls="icon-add" onclick="javascript:add()">新增部门</a>
						</c:if>
						<c:if test="${true || wface:contains(upList,'47') }">
							<a class="easyui-linkbutton" iconCls="icon-edit" onclick="javascript:edit()">修改部门</a>
						</c:if>
						<c:if test="${true || wface:contains(upList,'48') }">
							<a class="easyui-linkbutton" iconCls="icon-remove" onclick="javascript:removeAll()">删除部门</a>
						</c:if>
					</div>
				</form>
			</div>
			...
			<div id="addoreditdlg" title="  " class="easyui-dialog" data-options="draggable:true,resizable:true" style="width:500px;height:430px;padding:10px 20px" closed="true" buttons="#addoreditdlg-buttons">
				...
				<form id="addoreditfm" method="post" novalidate>
					<div class="fitem">
						<label>地址:</label>
						<input id="addr" name="addr" class="easyui-validatebox" required="true"  ><br/>
						<label>组ID:</label>
