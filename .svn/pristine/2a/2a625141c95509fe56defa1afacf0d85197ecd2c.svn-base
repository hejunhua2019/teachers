			...
			$(function() {
				...
				$(datagridId).datagrid({  
					...
					columns:[[
						...
						{field:'pGID',title:'上级组ID',width:60}						
					]],
			...
			<div id="toolbar_listTable" style="padding:5px;height:auto">
				...
				<form action="" id="searchForm">
					<div>
						...
						类型: <input name="type" style="width:60px">	
						上级组ID: <input name="pGID" style="width:60px">								
						<a class="easyui-linkbutton" iconCls="icon-search" onclick="javascript:doSearch()">查询</a>
						<c:if test="${true || wface:contains(upList,'46') }">
							<a class="easyui-linkbutton" iconCls="icon-add" onclick="javascript:add()">新增组</a>
						</c:if>
						<c:if test="${true || wface:contains(upList,'47') }">
							<a class="easyui-linkbutton" iconCls="icon-edit" onclick="javascript:edit()">修改组</a>
						</c:if>
						<c:if test="${true || wface:contains(upList,'48') }">
							<a class="easyui-linkbutton" iconCls="icon-remove" onclick="javascript:removeAll()">删除组</a>
						</c:if>
					</div>
				</form>
			</div>
