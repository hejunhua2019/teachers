...
<html>
	<head>
		...
		<script type="text/javascript">
			...
			var pageSize=20;
			var listUrl="groupSysResource/listpage";
			var updateUrl="groupSysResource/update";
			var deleteUrl="groupSysResource/deleteUpdateAll";
			var addUrl="groupSysResource/add";
			var url;
						...
						if (r){
							$.get(deleteUrl,{ids:ids},function(result){
								if (result["success"]==true){
									$(datagridId).datagrid('reload'); // reload the groupSysResource data
								} else {
			...
			function edit(){
				function switch2edit(){
					url=updateUrl;
					$(addOrEditFormId).form("reset");//重置表单
					$("#addoredittitle").html('编辑组资源');
				}
				switch2edit();				
				
				var rows = $(datagridId).datagrid('getSelections');
				if (rows.length == 0) {
					$.messager.alert('提示', "请选择你要更新的组资源", 'info');
					return;
				}
				if (rows.length > 1) {
					$.messager.alert('提示', "只能选择一个组资源进行更新", 'info');
					return;
				}
								
				var row = $(datagridId).datagrid('getSelected');
				if (row){
					$.get("groupSysResource/edit",{id:row.id},function(result){
						if (result["success"]==true){							
	...
	<body>
		...
		<div  class="easyui-panel"  title="组资源管理" >			
				...
				<form action="" id="searchForm">
					<div>
						...
						<c:if test="${true || wface:contains(upList,'46') }">
							<a class="easyui-linkbutton" iconCls="icon-add" onclick="javascript:add()">新增组资源</a>
						</c:if>
						<c:if test="${true || wface:contains(upList,'47') }">
							<a class="easyui-linkbutton" iconCls="icon-edit" onclick="javascript:edit()">修改组资源</a>
						</c:if>
						<c:if test="${true || wface:contains(upList,'48') }">
							<a class="easyui-linkbutton" iconCls="icon-remove" onclick="javascript:removeAll()">删除组资源</a>
						</c:if>
					</div>
				</form>
			</div>
