...
<html>
	<head>
		...
		<script type="text/javascript">
			...
			var addTitle="新增用户资源";
			var editTitle="编辑用户资源";
			var deleteConfirmMessage="你确定要删除吗?";
			var noSelectedRowMessage="请选择记录行";
			var searchFormId="#searchForm";
			var pageSize=20;
			var listUrl="userSysResource/listpage";
			var updateUrl="userSysResource/update";
			var deleteUrl="userSysResource/deleteUpdateAll";
			var addUrl="userSysResource/add";
			var url;
						...
						if (r){
							$.get(deleteUrl,{ids:ids},function(result){
								if (result["success"]==true){
									$(datagridId).datagrid('reload'); // reload the userSysResource data
								} else {
			...
			function edit(){
				function switch2edit(){
					url=updateUrl;
					$(addOrEditFormId).form("reset");//重置表单
					$("#addoredittitle").html('编辑用户资源');
				}
				switch2edit();				
				
				var rows = $(datagridId).datagrid('getSelections');
				if (rows.length == 0) {
					$.messager.alert('提示', "请选择你要更新的用户资源", 'info');
					return;
				}
				if (rows.length > 1) {
					$.messager.alert('提示', "只能选择一个用户资源进行更新", 'info');
					return;
				}
								
				var row = $(datagridId).datagrid('getSelected');
				if (row){
					$.get("userSysResource/edit",{id:row.id},function(result){
						if (result["success"]==true){							
	...
	<body>
		...
		<div  class="easyui-panel"  title="用户资源管理" >			
			...
			<div id="toolbar_listTable" style="padding:5px;height:auto">
				...
				<form action="" id="searchForm">
					<div>
						...
						资源ID: <input name="sysResourceID" style="width:60px">
						<a class="easyui-linkbutton" iconCls="icon-search" onclick="javascript:doSearch()">查询</a>
						<c:if test="${true || wface:contains(upList,'46') }">
							<a class="easyui-linkbutton" iconCls="icon-add" onclick="javascript:add()">新增用户资源</a>
						</c:if>
						<c:if test="${true || wface:contains(upList,'47') }">
							<a class="easyui-linkbutton" iconCls="icon-edit" onclick="javascript:edit()">修改用户资源</a>
						</c:if>
						<c:if test="${true || wface:contains(upList,'48') }">
							<a class="easyui-linkbutton" iconCls="icon-remove" onclick="javascript:removeAll()">删除用户资源</a>
						</c:if>
					</div>
				</form>
			</div>
			...
			<div id="addoreditdlg" title="  " class="easyui-dialog" data-options="draggable:true,resizable:true" style="width:500px;height:430px;padding:10px 20px" closed="true" buttons="#addoreditdlg-buttons">
				...
				<form id="addoreditfm" method="post" novalidate>
					<div class="fitem">
						...
						<label>资源ID:</label>
						<input id="sysResourceID" name="sysResourceID" class="easyui-validatebox" required="true"  ><br/>
					</div>					
